...
class ModelinesTest(TestCase):
    ...
    def _gen_raw_options_test(self, line, expected):
        ...
        if isinstance(line, list): self.assertEqual([x for x in sublime_modelines.gen_raw_options( line )], expected)
        else:                      self.assertEqual([x for x in sublime_modelines.gen_raw_options([line])], expected)
